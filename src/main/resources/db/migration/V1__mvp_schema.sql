...
CREATE TABLE job_task (id UUID PRIMARY KEY, task_type VARCHAR(40) NOT NULL, operation VARCHAR(24) NOT NULL, status VARCHAR(24) NOT NULL, document_id UUID NOT NULL REFERENCES knowledge_document(id), target_revision BIGINT NOT NULL, idempotency_key VARCHAR(120) NOT NULL, created_at TIMESTAMP WITH TIME ZONE NOT NULL, updated_at TIMESTAMP WITH TIME ZONE NOT NULL, UNIQUE(document_id, target_revision, operation, idempotency_key));
